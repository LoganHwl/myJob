...
$(function(){
    ...
    $.getJSON(ApiUrl + '/index.php?act=member_order&op=order_info',{key:key,order_id:getQueryString('order_id')}, function(result) {
        ...
        $('.pintuan_info').click(function () {
            var order_type = result.datas.order_info.order_type;
            if (order_type == 4) {
                var pintuan_info = result.datas.order_info.pintuan_info;
                var pintuan_id = pintuan_info.log_id;
                //if (pintuan_info.buyer_type > 0) pintuan_id = pintuan_info.buyer_type;
                location.href = WapSiteUrl + '/tmpl/pintuan_info.html?pintuan_id=' + pintuan_id +'&buyer_id=' + pintuan_info.buyer_id;
            }
        });
        $.ajax({
            type: 'post',
